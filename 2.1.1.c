...
for(i = 0; i < n; i++) {
	...
	for(i = 0; i < n - 1; i++) {  // Loop for n-1 iterations
	    for(j = 0; j < n - i - 1; j++) {  // Compare adjacent strings
	        if(strcmp(a[j], a[j+1]) > 0) {  // If the current string is greater than the next
	            strcpy(temp, a[j]);  // Swap the strings
	            strcpy(a[j], a[j+1]);
	            strcpy(a[j+1], temp);
	        }
	    }
